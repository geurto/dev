...
The `install.sh` script installs basic system dependencies, i3 window manager with polybar, and alacritty as a terminal emulator. Finally, it copies the configurations for these applications to `~/.config/`. For intended alacritty use, make sure that `$SHELL` evaluates to the Nix-installed `zsh`, e.g. `~/.nix-profile/bin/zsh` or `/nix/store/.../bin/zsh`.
