...
The `install.sh` script installs basic system dependencies, i3 window manager with polybar, and alacritty as a terminal emulator. Finally, it copies the configurations for these applications to `~/.config/`. For intended alacritty use, make sure that `terminal-tools` is installed as described above. The `install.sh` script will create a symlink from `/opt/bin/zsh` pointing to `~/.nix-profile/bin/zsh`. This is the only way I've found to not need to specify hardcode the username in `/home/<USER>/.nix-profile/bin/zsh` for Alacritty.
